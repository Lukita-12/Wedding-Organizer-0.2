<x-layout-form>
    ...
    <x-form.container variant="main">
        <x-form.form action="{{ route('admin.pembayaran.update', $pembayaran) }}" enctype="multipart/form-data">
            @method('PUT')
            ...
            <x-form.container variant="form">
                    ...
                    <x-form.select name="pesanan_id" id="pesanan_id">
                        ...
                        @foreach ($pesanans as $pesanan)
                            <option value="{{ $pesanan->id }}"
                                {{ (string) old('pesanan_id', $pembayaran->pesanan_id) === (string) $pesanan->id ? 'selected' : '' }} >
                ...
                <div>
                    ...
                    <x-form.input type="date" name="tgl_pembayaran" id="tgl_pembayaran" :value="old('tgl_pembayaran', optional($pembayaran->tgl_pembayaran)->format('Y-m-d'))" />
                ...
                <div class="grid grid-cols-2 gap-3">
                    ...
                    <div>
                        ...
                        <x-form.select name="bayar_dp" id="bayar_dp">
                            <option value="Belum dibayar" {{ old('bayar_dp', $pembayaran->bayar_dp) === 'Belum dibayar' ? 'selected' : '' }}>Belum dibayar</option>
                            <option value="Sudah dibayar" {{ old('bayar_dp', $pembayaran->bayar_dp) === 'Sudah dibayar' ? 'selected' : '' }}>Sudah dibayar</option>
                        </x-form.select>
                    ...
                    <div>
                        ...
                        <x-form.select name="bayar_lunas" id="bayar_lunas">
                            <option value="Belum dibayar" {{ old('bayar_lunas', $pembayaran->bayar_lunas) === 'Belum dibayar' ? 'selected' : '' }}>Belum dibayar</option>
                            <option value="Sudah dibayar" {{ old('bayar_lunas', $pembayaran->bayar_lunas) === 'Sudah dibayar' ? 'selected' : '' }}>Sudah dibayar</option>
                        </x-form.select>
