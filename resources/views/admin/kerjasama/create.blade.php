        ...
        <x-form.form action="{{ route('admin.kerjasama.store') }}">
            <div class="flex flex-col gap-3">
                <div class="flex flex-col gap-1">
                    <label for="noTelp_usaha" class="block px-4 poppins-semibold text-slate-700 text-xl">Pemilik & Nama Usaha</label>
                    <x-form.select name="request_mitra_id" id="request_mitra_id" required>
                        @foreach ($requestMitras as $requestMitra)
                            <option value="{{ $requestMitra->id }}">{{ $requestMitra->nama_pemilik }}, {{ $requestMitra->nama_usaha }}</option>
                        @endforeach
                    </x-form.select>
                ...
                <div class="flex flex-col gap-1">
                    ...
                    <input type="text" name="noTelp_usaha" id="noTelp_usaha" placeholder="No. telpon/WA..." :value="old('noTelp_usaha')" class="bg-slate-100 w-full px-4 py-1 inter text-lg text-slate-700 rounded-full focus:outline-none focus:ring-2 focus:ring-teal-500">
                    <x-form.error errorFor="noTelp_usaha" />
                </div>
                ...
                <div>
                    ...
                    <textarea type="text" name="alamat_usaha" id="alamat_usaha" placeholder="Alamat usaha..." class="bg-slate-100 w-full px-4 py-1 inter text-slate-700 text-lg rounded-lg focus:outline-none focus:ring-2 focus:ring-teal-500 resize-none" rows="4" required>{{ old('alamat_usaha') }}</textarea>
                    <x-form.error errorFor="alamat_usaha" />
                </div>

                <div>
                    <x-form.label for="harga01">Harga 01 Rp.</x-form.label>
                    <x-form.input type="text" name="harga01" id="harga01" step="0.01" min="0" :value="old('harga01')" placeholder="999.999.999" oninput="formatRupiah(this)" required />
                    <x-form.error errorFor="harga01" />
                </div>
    
                <div>
                    <x-form.label for="ket_harga01">Keterangan harga 01</x-form.label>
                    <x-form.textarea type="text" name="ket_harga01" id="ket_harga01" placeholder="Keterangan harga 01..." required>{{ old('ket_harga01') }}</x-form.textarea>
                    <x-form.error errorFor="ket_harga01" />
                </div>

                <div>
                    <x-form.label for="harga02">Harga 02 Rp.</x-form.label>
                    <x-form.input type="text" name="harga02" id="harga02" step="0.01" min="0" :value="old('harga02')" placeholder="999.999.999" oninput="formatRupiah(this)" required />
                    <x-form.error errorFor="harga02" />
                </div>
    
                <div>
                    <x-form.label for="ket_harga02">Keterangan harga 02</x-form.label>
                    <x-form.textarea type="text" name="ket_harga02" id="ket_harga02" placeholder="Keterangan harga 02..." required>{{ old('ket_harga02') }}</x-form.textarea>
