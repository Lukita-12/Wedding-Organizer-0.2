                ...
                <div>
                    ...
                    <div id="gambar_promosi_preview" class="flex gap-2 flex-wrap border-2 border-slate-500 border-dashed p-3 rounded">
                        ...
                        @foreach ($kerjasama->gambarPromosi as $gambar)
                            <div class="relative w-48 h-48 border border-slate-300 rounded overflow-hidden">
                                <img src="{{ asset('storage/' . $gambar->file_path) }}" alt="Gambar Promosi" class="object-cover w-full h-full">
                                <!-- Tombol hapus -->
                                <form action="{{ route('admin.gambar_promosi.destroy', $gambar) }}" method="POST" class="w-full flex justify-end absolute top-0 px-2 py-1 bg-white/30 backdrop-blur-sm">
                                    @csrf
                                    @method('DELETE')
                                    <button type="submit" class="poppins-semibold bg-red-500 text-white text-center text-xs w-6 h-6 rounded-sm hover:bg-red-600">X</button>
                                </form>
                            </div>
                        @endforeach

                        <!-- Kotak + Tambah Gambar -->
                        <div id="tambah-gambar-box" class="w-48 h-48 flex justify-center items-center bg-slate-100 rounded hover:bg-slate-200 transition cursor-pointer"
    ...
    function addNewImage(input) {
        if (input.files && input.files[0]) {
            ...
            reader.onload = function (e) {
                ...
                imgWrapper.className = "relative w-48 h-48 border border-slate-300 rounded overflow-hidden";
